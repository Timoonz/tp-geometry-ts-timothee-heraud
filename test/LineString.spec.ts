...
describe("test Point", () => {
    ...
    it("test constructor with ONE point, should be undefined", () => {
        const c = new Point([1.0,1.0]);
        const h = new LineString([c]);
        expect(h.getType()).to.equal("LineString");
        expect(h.getNumPoints()).to.equal(0);
        expect(h.getPointN(10)).to.equal(undefined);
    });
});
